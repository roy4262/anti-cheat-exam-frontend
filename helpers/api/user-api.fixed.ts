...
const submitExam = async (
  ...
  try {
    ...
    if (faceDetectionStats) {
      try {
        // Check if totalWarnings exists and add it to the validated stats
        const totalWarnings = typeof faceDetectionStats.totalWarnings === 'number'
          ? faceDetectionStats.totalWarnings
          : (faceDetectionStats.totalWarnings ? parseInt(String(faceDetectionStats.totalWarnings)) : 0);
        ...
        validatedStats = {
          ...
          // Add totalWarnings if it exists

...
const AppBarExam: React.FC<AppBarExamProps> = ({ examName, loadingBarRef }) => {
  ...
  const terminateExamDueToViolations = useCallback(() => {
    ...
    if (isLoading || submissionInProgressRef.current) {
      console.log('Submission already in progress, ignoring duplicate termination request');
      return;
    }

    console.log('EXECUTING EXAM TERMINATION DUE TO TAB SWITCHING VIOLATIONS');

    // Show a prominent error message
    toast.error("EXAM TERMINATED: Excessive tab switching detected (more than 3 times)!", {
      position: "top-center",
      autoClose: 5000,
    ...
    // Mark the violation in localStorage for redundancy
    if (activeExam?.exam?._id) {
      try {
        localStorage.setItem(`exam_${activeExam.exam._id}_terminated_for_tab_switching`, 'true');
        localStorage.setItem(`exam_${activeExam.exam._id}_termination_time`, new Date().toISOString());
      } catch (e) {
        console.error('Error storing termination data in localStorage:', e);
      }
    }

    // Force submission with the termination flag
    onEndExam(true);
  }, [onEndExam, isLoading, activeExam]);
  ...
  useEffect(() => {
    ...
    if (storedSwitches) {
      ...
      if (!isNaN(parsedCount)) {
        ...
        console.log(`Initialized tab switch count from localStorage: ${parsedCount}`);

        // Check if we should already terminate
        if (parsedCount >= 3) {
          console.log(`CRITICAL: Tab switch count (${parsedCount}) already exceeds limit of 3`);

          // Check if the exam was already terminated for tab switching
          const wasTerminated = localStorage.getItem(`exam_${examId}_terminated_for_tab_switching`);

          if (wasTerminated) {
            console.log('Exam was already terminated for tab switching, showing notification');
            toast.error("This exam was previously terminated due to excessive tab switching.");
          } else {
            ...
            setTimeout(() => {
              console.log('Executing delayed termination due to pre-existing tab switch violations');
      ...
      console.log('No stored tab switch count found, initializing to 0');
      // Initialize localStorage with 0 to ensure it exists
      localStorage.setItem(`exam_${examId}_tab_switches`, '0');
    }
    ...
    const handleVisibilityChange = () => {
      ...
      if (document.hidden) {
        ...
        if (newCount === 1) {
          ...
          // Use a small timeout to ensure the UI updates before termination
          setTimeout(() => {
            // Double-check the count to ensure it's still valid
            const finalCount = tabSwitchCountRef.current;
            if (finalCount >= 3) {
              console.log(`TERMINATING EXAM: Tab switch count (${finalCount}) exceeds limit of 3`);
              terminateExamDueToViolations();
            } else {
              console.log(`Not terminating: Tab switch count is now ${finalCount}`);
            }
          }, 500); // Increased timeout to ensure UI updates
        }
      }
    };
